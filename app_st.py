...
from cpt_consensus import MarketConsensus
from cpt_nlp import YahooFinBERTSentiment
...
# === PAGE HEADER ===
st.title("🏛️ PANTHEON")
st.markdown("An integrated analytics workspace for your **Portfolio Analysis**.")

# === SIDEBAR NAVIGATION ===
st.sidebar.title("🏛️ PANTHEON")
page = st.sidebar.radio(
    "Choose analysis module:",
    ["My Portfolio", "General Portfolio Analysis", "Equity Analysis", "Price Technical Analysis", "Machine Learning"]
)
...
# =========================================================
    ...
    if st.sidebar.button("Run Equity Analysis"):
        ...
        if not l_stocks:
            ...
            # =========================================================
            # 📰 NLP SENTIMENT (from cpt_nlp) — FinBERT on Yahoo articles
            # =========================================================
            st.subheader("📰 News Sentiment (Yahoo FinBERT)")

            try:
                with st.spinner("📰 Running FinBERT sentiment on recent Yahoo Finance news..."):
                    scorer = YahooFinBERTSentiment()

                    # If you added the sidebar controls above:
                    df_sentiment = scorer.portfolio(
                        l_stocks,
                        keywords=None,
                    )

                if df_sentiment is None or (hasattr(df_sentiment, "empty") and df_sentiment.empty):
                    st.info("No sentiment results returned (no news found or scoring returned empty output).")
                else:
                    st.dataframe(df_sentiment, use_container_width=True)

            except Exception as e:
                st.error(f"❌ Error running YahooFinBERTSentiment: {e}")

            # =========================================================
            # 📌 MARKET CONSENSUS (from cpt_consensus)
            # =========================================================
            st.subheader("🌐 Portfolio Market Consensus")

            try:
                mc = MarketConsensus(l_stocks)
                mc.build_portfolio_consensus()

                if hasattr(mc, "df_consensus") and mc.df_consensus is not None:
                    st.dataframe(mc.df_consensus, use_container_width=True)
                else:
                    st.info("MarketConsensus ran, but no df_consensus attribute was found.")

            except Exception as e:
                st.error(f"❌ Error running MarketConsensus: {e}")
...
# =========================================================
# 📈 3️⃣ Price Technical Analysis PAGE
# =========================================================
elif page == "Price Technical Analysis":
    st.header("⚡ Price Technical Analysis")
