...
class HMMRegimeModel:
    ...
    def fit(self):
        ...
        # Compute regime stats (sorted by momentum = bull/neutral/bear)
        regime_stats = (
        ...
        # -----------------------------------------------------
        # Map numeric regimes -> market meaning
        # -----------------------------------------------------
        ordered_states = list(regime_stats.index)  # sorted by Mom desc
        labels_map = {}
        ...
        if len(ordered_states) == 1:
            ...
            # best momentum -> bull, worst -> bear, others -> neutral
            labels_map[ordered_states[0]] = "bull"
            labels_map[ordered_states[-1]] = "bear"
            for s in ordered_states[1:-1]:
                labels_map[s] = "neutral"

        # add label column to df and stats
    ...
    def backtest(self):
        ...
        # Bull regime = first in regime_stats (highest momentum)
        bull_state = self.regime_stats.index[0]

        df["Position"] = (df["Regime"] == bull_state).astype(int)
        df["Position"] = df["Position"].shift(1).fillna(0)

        df["StrategyRet"] = df["Position"] * df["Return"]
        df["BH_Ret"] = df["Return"]

        df["Equity_Strategy"] = np.exp(df["StrategyRet"].cumsum())
        df["Equity_BuyHold"] = np.exp(df["BH_Ret"].cumsum())

        df["PnL_Strategy_%"] = (df["Equity_Strategy"] - 1.0) * 100
        df["PnL_BuyHold_%"] = (df["Equity_BuyHold"] - 1.0) * 100

        # Summary metrics (MONTHLY annualization)
        ...
        vol_strat = df["StrategyRet"].std() * np.sqrt(ann_factor)
        vol_bh    = df["BH_Ret"].std()       * np.sqrt(ann_factor)
...
if __name__ == "__main__":
    ...
    model = HMMRegimeModel("MSFT", "2015-01-01", n_states=2, freq="M")
